...
class Base:
    ...
    def get_requests(self, url: str, params: Dict = None):
        try:
            ...
            raise Exception(f'Не известная ошибка при запросе. url{url}, param:{params}, error:{e}')
        ...
        raise Exception(f'Получен не известный код ответа от github api. '
...
def parser_date(date: str):
    try:
        return datetime.datetime.strptime(date, '%d.%m.%Y')
    except ValueError:
        raise Exception(f'Неверный вормат даты: {date}')


def most_active_participant(arguments):
    MostActiveParticipant(url_public_repository=arguments.url, start_date_analysis=arguments.start_date,
                          end_date_analysis=arguments.end_date, branch_repository=arguments.branch)()


def count_pull_requests(arguments):
    CountPullRequests(url_public_repository=arguments.url, start_date_analysis=arguments.start_date,
                      end_date_analysis=arguments.end_date, branch_repository=arguments.branch).count_open_and_closed_pr()


def count_old_pull_requests(arguments):
    CountPullRequests(url_public_repository=arguments.url, start_date_analysis=arguments.start_date,
                      end_date_analysis=arguments.end_date, branch_repository=arguments.branch).count_old_pr()


def count_issues(arguments):
    CountIssues(url_public_repository=arguments.url, start_date_analysis=arguments.start_date,
                end_date_analysis=arguments.end_date).count_open_and_closed_issue()


def count_old_issues(arguments):
    CountIssues(url_public_repository=arguments.url, start_date_analysis=arguments.start_date,
                end_date_analysis=arguments.end_date).count_old_issue()
...
def create_parser():
    ...
    parser.add_argument('-b', '--branch', default='master', type=str, help='Ветка репозитория.')
    return parser
